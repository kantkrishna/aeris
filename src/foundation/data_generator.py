...
def generate_logistics_data(output_dir: str, num_customers: int = 100, num_orders: int = 500) -> None:
    """
    ...
    # 1. Generate Carriers
    carriers: List[Dict[str, Any]] = [{"carrier_id": str(uuid.uuid4()), "name": f"Carrier {i}"} for i in range(5)]
    
    # 2. Generate Customers
    customers: List[Dict[str, Any]] = [
        {"customer_id": str(uuid.uuid4()), "name": f"Customer {i}", "email": f"cust{i}@example.com"}
    ...
    _write_csv(out_path / "orders.csv", ["order_id", "customer_id", "order_date", "amount"], orders)
    _write_csv(out_path / "shipments.csv", ["shipment_id", "order_id", "carrier_id", "ship_date", "delivery_date", "status"], shipments)
