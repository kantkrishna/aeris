...
class IngestionEngine:
    ...
    def _validate_and_read_csv(
        self, file_path: str, max_length: int
        ...
        for row in data:
            if len(row) != len(headers):
                raise IngestionError(
                    f"Data mismatch: Row has {len(row)} columns, expected {len(headers)}"
                )
            for col in row:
                if max_length and len(str(col)) > max_length:
                    raise IngestionError(
                        f"Value '{col}' exceeds max length of {max_length}"
                    )

        return headers, data

    def ingest_raw_batch(
        self, file_path: str, table_name: str, max_length: int = 0
    ) -> None:
        """
    ...
    def ingest_staging_incremental(
        self, file_path: str, table_name: str, pk_cols: List[str]
    ) -> None:
        """
        ...
        try:
            with self.conn:
                self.conn.execute(
                    f"CREATE TABLE IF NOT EXISTS {table_name} ({cols_def}, PRIMARY KEY ({pk_def}))"
                )
                query = f"""
